...
import { AIRPORTS, AIRPORT_BY_IATA, vooPermitido } from '../src/game/data/airports'
...
for (let day = 0; day < DAYS; day++) {
  ...
  if (day % 10 === 0) {
    while (ti < targets.length) {
      const t = targets[ti]
      // A pista tem que ser conferida pelo mesmo portão do jogo: `runway` é a
      // decolagem no MTOW, e comparar com ela reprovava aeroporto de pista
      // curta que opera todo dia — com base em Santos Dumont o script não
      // abria rota nenhuma.
      const affordable = AIRCRAFT.filter(
        (a) => a.range > t.dist * 1.05 &&
          pistaServe(a, AIRPORT_BY_IATA[HUB], AIRPORT_BY_IATA[t.iata]) &&
...
if (first) {
  ...
  console.log(`\nrota exemplo ${first.from}-${first.to} (${km(first.distance)}, ${first.freq[1]}x/dia)`)
...
const t0 = targets[0]
console.log('\nalvo #1:', t0.iata, km(t0.dist) + ', demanda', Math.round(t0.total))
