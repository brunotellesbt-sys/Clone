...
export interface Airport {
  ...
  tetoAssentos?: number
  /**
   * A pista que a conta de operação usa, quando difere da real. Ver
   * `PISTA_OPERACIONAL`.
   */
 ...
 */
/**
 * A pista que a **operação** usa, quando ela não é a pista do aeroporto.
 *
 * Dois papéis moravam no mesmo campo e brigavam. `runway` é um fato que a tela
 * mostra: Santos Dumont tem 1.323 m, e é isso que o menu de porte deve dizer.
 * Mas a conta de quem pousa ali também lia `runway`, e para o teto de porte
 * funcionar alguém precisou inflar o número — os 47 aeroportos com teto
 * carregavam cinco valores sintéticos repetidos por faixa, e Santos Dumont
 * aparecia com 2.149 m.
 *
 * Aqui ficam esses números, agora com o nome do que eles são: a régua de
 * operação herdada, que **não muda nenhuma aeronave aceita**. A pista de
 * verdade voltou para `runway`, onde o jogador a lê.
 *
 * Não é uma solução bonita, é uma solução honesta: enquanto a régua de
 * operação for herdada em vez de calculada, ela fica num campo com nome
 * próprio, e não disfarçada de comprimento de pista.
 */
export const PISTA_OPERACIONAL: Record<string, number> = {
  ARU: 7050,
  ATM: 7050,
  AUX: 7050,
  BPS: 8450,
...
PIN|Parintins|Brasil|BR|-2.67|-56.78|1.04|0.68|0.85|5906|2|1348
...
/**
 * Os pares que **são** voo apesar de o sistema juntar os dois aeroportos.
 *
 * O satélite distante não é o mesmo destino que o centro. Southend fica a 80 km
 * de Heathrow, Bergamo a 76 de Malpensa, Ontario a 76 de Los Angeles: são
 * aeroportos de cidade própria que a companhia vende como alternativa barata ao
 * metrô, e voar entre eles é voo — enquanto Heathrow–Gatwick, que está no mesmo
 * grupo, continua não sendo.
 *
 * A lista é revisão do dono do projeto, par a par, e é por isso que ela não é
 * derivada de distância: Los Angeles–Santa Ana, a 58 km, continua proibido, e
 * Ontario–Santa Ana, a 49, é voo. Quem decide é quem conhece o mercado, não a
 * régua.
 */
const EXCECOES_DE_SISTEMA = new Set([
  // Londres: Southend é cidade própria, longe do cinturão de Heathrow
  'LCY-SEN', 'LGW-SEN', 'LHR-SEN', 'LTN-SEN', 'SEN-STN',
  // Los Angeles: o centro segue ligado a Ontario e a Santa Ana por terra,
  // mas os satélites entre si são voo
  'LAX-ONT', 'BUR-ONT', 'LGB-ONT', 'ONT-SNA', 'BUR-SNA', 'LGB-SNA',
  // Estocolmo, Oslo, Milão, Cidade do México e Washington
  'ARN-NYO', 'BMA-NYO', 'NYO-VST',
  'OSL-TRF',
  'BGY-MXP', 'BGY-LIN',
  'NLU-TLC',
  'BWI-DCA',
...
/** A chave do par, com as pontas em ordem, para a busca não depender do sentido. */
const parDe = (a: string, b: string) => (a < b ? `${a}-${b}` : `${b}-${a}`)

export function mesmoSistemaAeroportuario(a: Airport, b: Airport): boolean {
  if (a.iata === b.iata) return false
  if (EXCECOES_DE_SISTEMA.has(parDe(a.iata, b.iata))) return false
