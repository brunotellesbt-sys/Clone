  ...
  chooseWhoWillEvolve(eventSource: EventSource): void {
    ...
    if (this.auxPokemonList.length === 0) {
      switch (eventSource) {
	    ...
	    // Gen 9 special trainer for Battle Trainer encounters
	    // Use existing GenerationService API (getCurrentGeneration) to avoid TS2551.
	    if (eventSource === 'battle-trainer' && this.generationService.getCurrentGeneration().id === 9) {
	      this.altPrizeText = 'Cassiopeia';
	      // Cassiopeia is Penny's alias in Pokémon Scarlet/Violet
	      this.altPrizeSprite = 'https://play.pokemonshowdown.com/sprites/trainers/penny.png';
	      this.altPrizeDescription = 'Cassiopeia challenges you to a battle!';
	      this.modalService.open(this.altPrizeModal, {
	        centered: true,
	        size: 'md'
	      });
